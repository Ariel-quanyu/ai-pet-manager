...
import { getStoredCity,isLocationPermissionError,locateCurrentCity } from '@/services/location-service'
import type { Pet } from '@/domain/pet'
import './index.scss'
const shortcuts=[['▣','AI宠物管家'],['▤','计次卡'],['✪','积分商城'],['▱','其他']]
export default function HomePage(){
 const [pets,setPets]=useState<Pet[]>(petRepository.getCached())
 ...
 const showLocationFailure=async(reason:unknown)=>{
  ...
  try{
   const result=await Taro.showModal({
    title:'定位失败',content:`${message}。首页其他功能不受影响。`,
 ...
 useDidShow(()=>{
  void petRepository.list().then(setPets).catch(reason=>{
   const message=reason instanceof Error?reason.message:'宠物数据加载失败'
