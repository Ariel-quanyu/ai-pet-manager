...
export default function AuthPage(){
 const finish=(event?: NonNullable<ButtonProps['onGetPhoneNumber']> extends (...args: infer A)=>unknown ? A[0] : never)=>{ if(event?.detail?.errMsg?.includes('fail')) Taro.showToast({title:'已进入体验模式',icon:'none'}); Taro.redirectTo({url:'/features/home/index'}) }
